...
namespace System.Windows.Forms
{
    ...
    public class CustomTabControl : TabControl
    {
        ...
        public bool ShowTabTitleCenter { get; set; }
        ...
        protected override void OnHandleCreated(EventArgs e)
        {
            ...
            SendMessage(this.Handle, 0x1300 + 49, IntPtr.Zero, (IntPtr)1);
        }
        ...
        protected virtual void CustomPaint(Graphics screenGraphics)
        {
        ...
        protected virtual void DrawTabPage(int index, Graphics graphics)
        {
            graphics.SmoothingMode = SmoothingMode.HighSpeed;
            ...
            using (GraphicsPath tabPageBorderPath = this.GetTabPageBorder(index))
            {
                ...
                {
                    //	Paint the tab
                    this._StyleProvider.DrawTab(index, graphics);
        ...
        private void DrawTabText(int index, Graphics graphics)
        {
            ...
            {
                textBrush = new SolidBrush(this._StyleProvider.TextColorSelected);
            }
            else
            {
                if (this.TabPages[index].Enabled)
                {
                    textBrush = new SolidBrush(this._StyleProvider.TextColor);
                }
                else
                {
                    textBrush = new SolidBrush(this._StyleProvider.TextColorDisabled);
                }
            }

            graphics.DrawString(tab.Text, this.Font, textBrush, tabBounds, this.GetStringFormat());
            textBrush.Dispose();
        }
        ...
        private StringFormat GetStringFormat()
        {
            ...
            if (ShowTabTitleCenter) // 除非设置，否则不在中间显示
            {
            ...
            using (GraphicsPath path = this._StyleProvider.GetTabBorder(index))
            {
                ...
                switch (this.Alignment)
                {
                    case TabAlignment.Top:
                        ...
                        textRect.X = tab.Padding.Left + textRect.X;
                        textRect.Y = tab.Padding.Top + textRect.Y;
                        //textRect.Y += 4;
                        //textRect.Height -= 6;
                        break;
                    case TabAlignment.Bottom:
                ...
                if (this.ImageList != null
                    && (tab.ImageIndex > -1
                    || (!string.IsNullOrEmpty(tab.ImageKey)
                    && !tab.ImageKey.Equals("(none)", StringComparison.OrdinalIgnoreCase))))
                {
                    Rectangle imageRect = this.GetTabImageRect(index);
